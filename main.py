...
heart = pygame.image.load('HUD/heart.png').convert_alpha()
...
#--------------BOSS ROOM (LVL 2)-------------------
    ...
    while boss1.alive():
        ...
        for x in range(0,player.health):
            screen.blit(heart, (0+(16*x), 0)) #Display HP.
        screen.blit(pygame.image.load(element[tabCount]).convert_alpha(), (0,16)) #Display element.
        #Now service player after everything else.
        player.update(pygame.key.get_pressed()) #Update player
        if (player.health>0 and player.invinc%2==0):
            screen.blit(player.image, player.rect) #Draw them if necessary
        ...
        if boss1.alive():
            ...
            if bulletLoops % 120 == 0:
                baby1 = enemy.enemy('Enemies/slime.png', (boss1.rect.x - 10, boss1.rect.y), [], "", enemies)       #spawn mini monsters
                baby2 = enemy.enemy('Enemies/slime.png', (boss1.rect.x + 10, boss1.rect.y), [], "", enemies)
...
#-------------------------FINAL BOSS-------------------------
    ...
    while boss1.alive() and boss2.alive() and boss3.alive():        #just using the same boss for now, will change later
        ...
        #------Flashing invincibility------- created by Steven Goodchild
        allSprites.draw(screen)
        if boss1.alive():       #will end once a boss is dead, will fix this later
            healthBar = pygame.draw.rect(screen, (255, 0, 0), (220, 440, boss1.boss_health*10, 15))
            if bulletLoops % 120 == 0:
                baby1 = enemy.enemy('Enemies/slime.png', (boss1.rect.x - 10, boss1.rect.y), [], "", enemies)       #spawn mini monsters
                baby2 = enemy.enemy('Enemies/slime.png', (boss1.rect.x + 10, boss1.rect.y), [], "", enemies)
