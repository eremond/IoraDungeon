...
class player(sprites.sprites):
	...
	def update(self, keys):
		...
		if self.kb > 0:
			self.kb -= 1
			if (self.kbdx>0 and self.inBoundsRight()) or (self.kbdx<0 and self.inBoundsLeft()):
				self.rect.x+=self.kbdx
			if (self.kbdy>0 and self.inBoundsDown()) or (self.kbdy<0 and self.inBoundsUp()):
				self.rect.y+=self.kbdy
			return #Deny movement until knockback is over.
